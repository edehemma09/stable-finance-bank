...
1. **Move user authentication to the publishable-key client**
  ...
  - Remove all registration, resend, and forgot-password and login calls to the role-key-backed `sendAuthEmail` action.
2. **Preserve Admin SMTP through the secure auth-email hook**
  ...
  - The hook will not only deliver email; it will  create users, generate auth links, or perform authentication with the publishable key.
