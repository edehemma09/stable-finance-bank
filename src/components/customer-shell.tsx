...
const TABS = NAV.filter((n) =>
  ["/app", "/app/accounts", "/app/transfers", "/app/wealth"].includes(n.to),
);
    ...
    <div className="min-h-screen bg-surface text-foreground">
      ...
      <aside className="fixed inset-y-0 left-0 hidden w-64 flex-col border-r bg-sidebar text-sidebar-foreground md:flex">
        <div className="border-b border-sidebar-border p-5">
          <BrandMark className="text-sidebar-foreground [&_span]:text-sidebar-foreground" />
        </div>
        <nav className="flex-1 space-y-1 overflow-y-auto p-3">
          {NAV.map(({ to, label, icon: Icon, exact }) => (
            <Link
              key={to}
              to={to as never}
              className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm transition-colors ${isActive(to, exact) ? "bg-sidebar-accent text-sidebar-accent-foreground" : "text-sidebar-foreground/70 hover:bg-sidebar-accent/50 hover:text-sidebar-foreground"}`}
            >
          ...
          {roles.includes("admin") && (
            <Link
              to="/admin"
              className="mt-4 flex items-center gap-3 rounded-lg border border-sidebar-border/60 px-3 py-2 text-sm text-sidebar-primary hover:bg-sidebar-accent/40"
            >
      ...
      <div className="md:pl-64">
        ...
        <main className="min-h-[calc(100vh-4rem)] pb-28 md:min-h-screen md:pb-0">
          <Outlet />
        </main>
      </div>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-50 md:hidden">
        <div className="mx-3 mb-3 grid grid-cols-5 items-end rounded-2xl border border-border/60 bg-background/95 px-1 py-1.5 shadow-[0_-4px_24px_-8px_rgba(0,0,0,0.25)] backdrop-blur">
          {TABS.slice(0, 2).map((t) => (
            <Tab key={t.to} item={t} active={isActive(t.to, t.exact)} />
          ))}

          {/* Center action */}
          <Link to="/app/transfers" className="flex flex-col items-center gap-1 pb-1">
            <span
              className={`grid h-12 w-12 -translate-y-4 place-items-center rounded-2xl shadow-lg ring-4 ring-background transition-transform active:scale-95 ${isActive("/app/transfers") ? "bg-primary text-primary-foreground" : "bg-accent text-accent-foreground"}`}
            >
              <ArrowLeftRight className="h-5 w-5" />
            </span>
            <span className="-mt-3 text-[10px] font-semibold tracking-wide text-muted-foreground">
              Transfer
            </span>
          </Link>
          ...
          <Sheet open={moreOpen} onOpenChange={setMoreOpen}>
            <SheetTrigger asChild>
              <button
                className={`flex flex-col items-center gap-1 rounded-xl py-1.5 text-[10px] font-semibold transition-colors ${moreActive ? "text-accent" : "text-muted-foreground"}`}
              >
            ...
            <SheetContent side="bottom" className="rounded-t-3xl pb-8">
              <SheetHeader className="text-left">
                <SheetTitle className="font-display text-xl">All services</SheetTitle>
              </SheetHeader>
              ...
              {roles.includes("admin") && (
                <Link
                  to="/admin"
                  onClick={() => setMoreOpen(false)}
                  className="mt-4 flex items-center justify-between rounded-2xl border bg-primary p-4 text-sm font-semibold text-primary-foreground"
                >
              ...
              <div className="mt-3 flex items-center justify-between rounded-2xl border bg-card p-4">
                <div className="min-w-0">
                  ...
                  <p className="truncate text-xs text-muted-foreground">
                    Daily limit {formatUSD(profile?.transaction_limit ?? 5000)}
                  </p>
                </div>
...
function Tab({ item, active }: { item: NavItem; active: boolean }) {
  ...
  return (
    <Link
      to={item.to as never}
      className={`flex flex-col items-center gap-1 rounded-xl py-1.5 text-[10px] font-semibold transition-colors ${active ? "text-accent" : "text-muted-foreground"}`}
    >
      <Icon className="h-5 w-5" />
      {item.label}
    </Link>
