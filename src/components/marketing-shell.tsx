...
const SEGMENTS: {
  key: "personal" | "business" | "membership";
  label: string;
  groups: NavGroup[];
}[] = [
  {
        ...
        <div className="bg-primary text-primary-foreground">
          <div className="container-page flex h-16 items-center justify-between gap-4">
            ...
            <div className="hidden items-center gap-3 md:flex">
              <a
                href="/contact"
                className="inline-flex items-center gap-1.5 text-sm text-primary-foreground/85 hover:text-primary-foreground"
              >
          ...
          <div className="container-page flex h-12 items-center gap-7">
                ...
                <div className="pointer-events-none absolute left-0 top-full z-[100] mt-0 hidden min-w-[240px] rounded-b-md border border-border/70 bg-card p-2 shadow-lg group-hover:pointer-events-auto group-hover:block">
                  {g.items.map((it) => (
                    <Link
                      key={it.label}
                      to={it.to}
                      className="block rounded px-3 py-2 text-sm text-foreground hover:bg-surface"
                    >
                      {it.label}
                    </Link>
                  ))}
            ...
            {activeSeg === "membership" && (
              <Link
                ...
                className="ml-auto whitespace-nowrap text-sm font-semibold text-accent hover:underline"
              >
                Become a Member →
              </Link>
            )}
                ...
                return (
                  <div key={s.key} className="border-b border-border/60">
                    ...
                    >
                      {s.label}
                      <ChevronDown
                        className={`h-4 w-4 transition-transform ${segOpen ? "rotate-180" : ""}`}
                      />
                    </button>
                    {segOpen && (
                      <div className="pb-2">
                          ...
                          return (
                            <div key={g.label} className="border-t border-border/40">
                              ...
                              >
                                {g.label}
                                <ChevronDown
                                  className={`h-3.5 w-3.5 transition-transform ${gOpen ? "rotate-180" : ""}`}
                                />
                              </button>
                              {gOpen && (
      ...
      <footer className="bg-primary-dark text-primary-foreground">
        <div className="container-page grid gap-10 py-14 md:grid-cols-5">
          <div className="md:col-span-2">
            ...
            <p className="mt-4 max-w-sm text-sm text-primary-foreground/75">
              {settings?.tagline ?? "Banking built around your day."}
            </p>
            <p className="mt-4 text-xs text-primary-foreground/60">
              Routing #{settings?.routing_number ?? "000000000"} · Federally insured (illustrative)
            </p>
          </div>
          <FooterCol
            title="Personal"
        ...
        <div className="border-t border-primary-foreground/15">
          <div className="container-page py-4 text-xs text-primary-foreground/60">
            © {new Date().getFullYear()} {settings?.brand_name ?? "Stable Finance Bank"}. All rights
            reserved.
          </div>
        </div>
      </footer>
      ...
      <ul className="space-y-2 text-sm">
        {links.map((l) => (
          <li key={l.to + l.label}>
            <Link
              to={l.to}
              className="text-primary-foreground/85 hover:text-primary-foreground hover:underline"
            >
              {l.label}
            </Link>
          </li>
