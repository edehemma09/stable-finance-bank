  ...
  return (
    <div className="container-page py-6 md:py-10">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Move money between your accounts, to another Stable Finance member, or externally.
      </p>
      {receipt ? (
        <Receipt r={receipt} onDone={() => setReceipt(null)} />
      ) : (
        <Tabs defaultValue="same" className="mt-6">
          ...
          <TabsContent value="same">
            <SameForm accounts={accounts} onSuccess={setReceipt} />
          </TabsContent>
          <TabsContent value="intra">
            <IntraForm accounts={accounts} onSuccess={setReceipt} />
          </TabsContent>
          <TabsContent value="ach">
            <ExternalForm kind="ach" accounts={accounts} onSuccess={setReceipt} />
          </TabsContent>
          <TabsContent value="wire">
            <ExternalForm kind="wire" accounts={accounts} onSuccess={setReceipt} />
          </TabsContent>
        </Tabs>
      )}
    </div>
...
function useTransfer(
  onSuccess: (r: { id: string; amount: number; kind: string; to: string }) => void,
) {
...
function AccountSelect({
  ...
  accounts.forEach((a) =>
    accountsCache.set(a.id, `${a.nickname ?? a.type} ••${a.account_number.slice(-4)}`),
  );
  return (
    <div>
      <Label>{label}</Label>
...
function SameForm({
  ...
  return (
    <form
      onSubmit={(e) => {
        e.preventDefault();
        m.mutate({ from, to, kind: "same", amount: Number(amt), memo });
      }}
    ...
    >
      ...
      <Button type="submit" disabled={!from || !to || !amt || m.isPending} className="w-full">
        {m.isPending ? "Sending…" : "Transfer"}
      </Button>
    </form>
  );
}
...
function IntraForm({
  ...
  async function lookup() {
    ...
    if (!q) return;
    setLooking(true);
    const { data, error } = await (supabase.rpc as any)("resolve_transfer_recipient", {
      _query: q,
    });
    setLooking(false);
    ...
    if (error || !hit) {
      setFound(null);
      toast.error("No active Stable Finance member found for that account number or username");
      return;
    }
  ...
  return (
    <form onSubmit={submit} className="mt-4 space-y-4 rounded-xl border bg-card p-6">
      ...
      <div>
        ...
        <div className="flex gap-2">
          <Input
            ...
            onBlur={() => {
              if (target.trim() && !found) void lookup();
            }}
            required
        ...
        {found ? (
          <p className="mt-2 flex items-center gap-1.5 text-sm text-success">
            <CheckCircle2 className="h-4 w-4" /> Sending to{" "}
            <span className="font-medium">{found.display_name}</span> · {found.masked_account}
          </p>
        ) : (
          <p className="mt-2 text-xs text-muted-foreground">
            Routing number {site?.routing_number ?? ""} identifies {site?.brand_name ?? "the bank"}{" "}
            — use the member&apos;s account number or username to pick the recipient.
          </p>
        )}
      </div>
      <div>
      ...
      <Button type="submit" disabled={!from || !target || !amt || m.isPending} className="w-full">
    ...
    <form
      onSubmit={(e) => {
        ...
        m.mutate({
          ...
          amount: Number(amt),
          memo,
          external: { name, bank, account_number: account, routing_number: routing },
        });
      }}
      className="mt-4 space-y-4 rounded-xl border bg-card p-6"
    >
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div>
          ...
          <Input value={routing} onChange={(e) => setRouting(e.target.value)} required />
        </div>
      </div>
      <div>
        <Label>Amount</Label>
      ...
      <p className="text-xs text-muted-foreground">
        {kind === "wire"
          ? "Wire transfers settle same-day when submitted before 4:00 PM ET."
          : "ACH transfers typically settle in 1–3 business days."}
      </p>
      <Button
        type="submit"
        disabled={!from || !name || !account || !amt || m.isPending}
        className="w-full"
      >
        {m.isPending ? "Sending…" : `Send ${kind.toUpperCase()}`}
      </Button>
    </form>
  );
}
...
function Receipt({
  ...
  r: { id: string; amount: number; kind: string; to: string };
  onDone: () => void;
}) {
  const { data: site } = useQuery(siteSettingsQuery);
