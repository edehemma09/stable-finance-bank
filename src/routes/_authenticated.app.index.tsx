...
function Dashboard() {
  ...
  const secondary =
    accounts.find((a) => a.type === "savings" && a.id !== primary?.id) ??
  ...
  const seed = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success("Demo accounts opened");
    },
    ...
    <div className="mx-auto max-w-5xl px-5 py-6 lg:px-10 lg:py-10">
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Here is your account summary as of {dateLabel}.
          </p>
        </div>
        {accounts.length === 0 && (
          ...
          <div className="relative overflow-hidden rounded-2xl bg-primary p-6 text-primary-foreground shadow-lg">
            ...
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold uppercase tracking-widest text-accent">
                {primary.nickname ?? `${primary.type} account`}
              </span>
            ...
            <div className="mt-8 flex gap-3">
              <Link
                to="/app/transfers"
                className="flex-1 rounded-lg bg-accent py-2 text-center text-xs font-bold uppercase tracking-wider text-primary transition-colors hover:brightness-105"
              >
                Transfer
              </Link>
              <Link
                ...
                className="flex-1 rounded-lg bg-primary-foreground/10 py-2 text-center text-xs font-bold uppercase tracking-wider text-primary-foreground transition-colors hover:bg-primary-foreground/20"
              >
                View Details
              </Link>
            </div>
          ...
          <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold uppercase tracking-widest text-primary">
                {secondary.nickname ?? `${secondary.type} account`}
              </span>
              <span className="text-xs font-medium text-muted-foreground">
                …{secondary.account_number.slice(-4)}
              </span>
            </div>
            <div className="mt-4">
              <div className="font-display text-3xl font-bold text-primary">
                {formatUSD(secondary.balance)}
              </div>
      ...
      <div className="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border bg-card p-4">
        ...
        {profile?.kyc_status === "verified" ? (
          ...
          <Link
            to="/app/profile"
            className="rounded-lg bg-accent px-4 py-2 text-xs font-bold uppercase tracking-wider text-accent-foreground"
          >
            Verify identity to raise limit
          </Link>
        )}
      </div>
      ...
      <div className="mt-10">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <Module
            ...
            copy="Portfolio, allocation & advisory."
          />
          <Module
            to="/app/loans"
            icon={Landmark}
            title="Loans"
            copy="Apply, track and pay down balances."
          />
          <Module
            to="/app/retirement"
            icon={PiggyBank}
            title="Retirement & IRS"
            copy="IRA contributions and tax forms."
          />
        </div>
      </div>
      ...
      <div className="mt-10">
        <div className="mb-4 flex items-center justify-between">
          ...
          <Link to="/app/accounts" className="text-sm font-semibold text-accent hover:underline">
            View All
          </Link>
        </div>
