...
export const Route = createFileRoute("/_authenticated/admin/customers/$id")({
  loader: ({ context, params }) =>
    context.queryClient.ensureQueryData(adminCustomerQuery(params.id)),
...
function CustomerDetail() {
  ...
  useEffect(() => {
    if (!adj.account_id && data.accounts[0])
      setAdj((a) => ({ ...a, account_id: data.accounts[0]!.id }));
  ...
  const saveLimit = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.rpc("admin_set_transaction_limit", {
        _user_id: id,
        _limit: Number(limit),
      });
      if (error) throw error;
  ...
  const adjust = useMutation({
    mutationFn: async () => {
      ...
      if (!adj.account_id || !amount || amount <= 0)
        throw new Error("Enter a positive amount and pick an account");
  ...
  return (
    <div className="p-4 md:p-8">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-3">
        ...
        <span
          className={`rounded-full border px-2 py-0.5 text-[11px] capitalize ${statusTone(p.status)}`}
        >
          {p.status}
        </span>
              ...
              >
                <div>
                  ...
                  <p className="font-mono text-xs text-muted-foreground">
                    ••{a.account_number.slice(-4)} · {a.status}
                  </p>
                </div>
          ...
          <div className="mt-3 grid gap-3">
            <div>
              ...
              <select
                ...
                onChange={(e) => setAdj({ ...adj, account_id: e.target.value })}
              >
                {data.accounts.map((a) => (
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <select
                  ...
                  value={adj.direction}
                  onChange={(e) =>
                    setAdj({ ...adj, direction: e.target.value as "credit" | "debit" })
                  }
                >
            ...
            <div>
              ...
              <Input
                value={adj.memo}
                onChange={(e) => setAdj({ ...adj, memo: e.target.value })}
          ...
          <div className="mt-3 grid gap-3">
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              ...
              <Button
                size="sm"
                variant="destructive"
                onClick={() => setStatus.mutate("banned")}
                disabled={setStatus.isPending}
              >
                Ban
              </Button>
            ...
            <p className="text-xs text-muted-foreground">
              Suspending, banning or closing freezes every account owned by this customer. All
            ...
            <div className="mt-2 border-t pt-3">
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      This removes the customer&apos;s login, accounts, cards, transactions and
                      tickets. A snapshot is archived in the recycle bin and the action is written
                      to the audit log. You cannot delete your own account.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                Deleted customers stay recoverable as an archived snapshot in the recycle bin.
              </p>
            </div>
          </div>
        </div>
            ...
            <tbody>
              {data.transactions.map((t) => (
                <tr key={t.id} className="border-b last:border-0">
                  ...
                  <td
                    className={`px-5 py-3 text-right font-mono ${Number(t.amount) < 0 ? "text-destructive" : "text-success"}`}
                  >
                    {formatUSD(t.amount)}
                  </td>
                </tr>
              ...
              {data.transactions.length === 0 && (
                <tr>
                  <td className="px-5 py-8 text-center text-muted-foreground">No activity yet.</td>
                </tr>
              )}
            </tbody>
          </table>
