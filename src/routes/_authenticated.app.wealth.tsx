...
function Wealth() {
  ...
  const dayPnl = holdings.reduce(
    (s, h) => s + Number(h.shares) * Number(h.price) * (Number(h.day_change) / 100),
    0,
  );
  ...
  const open = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase
        .from("holdings")
        .insert(STARTER.map((h) => ({ ...h, user_id: u.id, account_id: brokerage?.id ?? null })));
      if (error) throw error;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["me", "holdings"] });
      toast.success("Managed portfolio opened");
    },
  ...
  holdings.forEach((h) => {
    byClass[h.asset_class] = (byClass[h.asset_class] ?? 0) + Number(h.shares) * Number(h.price);
  });
  ...
  return (
    <div className="mx-auto max-w-5xl px-5 py-6 lg:px-10 lg:py-10">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Portfolio, allocation, and advisory in one place.
      </p>
      ...
      <div className="mt-6 rounded-2xl bg-primary p-6 text-primary-foreground shadow-lg">
        ...
        <p
          className={`mt-1 flex items-center gap-1 text-sm ${dayPnl >= 0 ? "text-accent" : "text-primary-foreground/70"}`}
        >
      ...
      {holdings.length === 0 ? (
        <div className="mt-6 grid place-items-center rounded-2xl border border-dashed p-10 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            No investments yet. Open a managed portfolio to get started.
          </p>
          <Button className="mt-4" onClick={() => open.mutate()} disabled={open.isPending}>
                ...
                <div key={h.id} className="flex items-center justify-between gap-3 p-4">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {Number(h.shares)} @ {formatUSD(h.price)}
                    </p>
                  </div>
                  <div className="shrink-0 text-right">
                    <p className="text-sm font-bold">{formatUSD(val)}</p>
                    <p
                      className={`text-xs font-semibold ${up ? "text-success" : "text-destructive"}`}
                    >
                      {up ? "+" : ""}
                      {Number(h.day_change)}%
                    </p>
