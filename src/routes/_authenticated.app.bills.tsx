...
function Bills() {
  ...
  const addPayee = useMutation({
    mutationFn: async (v: { name: string; category: string; account_number: string }) => {
      ...
      const { error } = await supabase.from("payees").insert({ ...v, user_id: u.id });
      if (error) throw error;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["me", "payees"] });
      setAddOpen(false);
    ...
    mutationFn: async (v: {
      ...
      notifyByEmail({
        ...
        rows: [
          ...
          [
            "Paid from",
            from ? `${from.nickname ?? from.type} ••${from.account_number.slice(-4)}` : "—",
          ],
          ["Memo", v.memo || "—"],
  ...
  return (
    <div className="container-page py-6 md:py-10">
          ...
          >
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {p.category} · ••{p.account_number?.slice(-4)}
              </p>
            </div>
            <Button size="sm" onClick={() => setPayOpen(p.id)}>
              Pay
            </Button>
          </div>
        ))}
      ...
      <Dialog open={!!payOpen} onOpenChange={(v) => !v && setPayOpen(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Pay {payees.find((p) => p.id === payOpen)?.name}</DialogTitle>
          </DialogHeader>
          {payOpen && (
            <PayForm
              payeeId={payOpen}
              accounts={accounts}
              onSubmit={payBill.mutate}
...
function AddPayeeForm({
  ...
  onSubmit: (v: { name: string; category: string; account_number: string }) => void;
  pending: boolean;
}) {
  ...
  return (
    <form
      onSubmit={(e) => {
        e.preventDefault();
        onSubmit({ name, category: cat, account_number: acct });
      }}
      className="space-y-3"
    >
      <div>
      ...
      <div>
        ...
        <Select value={cat} onValueChange={setCat}>
          ...
          <SelectContent>
            {["utility", "rent", "credit-card", "loan", "insurance", "other"].map((c) => (
      ...
      <div>
        ...
        <Input value={acct} onChange={(e) => setAcct(e.target.value)} required />
      </div>
      <DialogFooter>
...
function PayForm({
  ...
  return (
    <form
      onSubmit={(e) => {
        e.preventDefault();
        onSubmit({ payee_id: payeeId, from_account_id: from, amount: Number(amt), memo });
      }}
      className="space-y-3"
    >
      <div>
          ...
          <SelectContent>
            {accounts.map((a) => (
              <SelectItem key={a.id} value={a.id}>
                {a.nickname ?? a.type} — {formatUSD(a.balance)}
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
      ...
      <DialogFooter>
        <Button type="submit" disabled={pending || !from || !amt}>
          {pending ? "Sending…" : "Pay"}
        </Button>
      </DialogFooter>
    </form>
