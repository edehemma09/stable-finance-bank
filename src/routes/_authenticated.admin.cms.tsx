  ...
  const { data } = useQuery({
    ...
    queryFn: async () =>
      (
        await supabase
          .from("pages")
          .select("id,slug,title,nav_label,nav_order,in_nav,published")
          .order("nav_order")
      ).data ?? [],
  });
  return (
    <div className="p-4 md:p-8">
        ...
        {(data ?? []).map((p) => (
          <div key={p.id} className="rounded-xl border bg-card p-4">
            ...
            <div className="mt-3 flex items-center gap-2 text-[11px]">
              <span
                className={`rounded-full border px-2 py-0.5 ${p.published ? "border-success/30 bg-success/10 text-success" : "text-muted-foreground"}`}
              >
      ...
      <p className="mt-6 text-sm text-muted-foreground">
        Block editor, logo and brand upload arrive in the CMS pass —{" "}
