...
function Profile() {
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("profiles").update(f).eq("id", u.id);
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <div className="rounded-xl border bg-card p-6">
          ...
          <div className="mt-4 grid gap-3">
            ...
            <Button onClick={() => save.mutate()} disabled={save.isPending} className="mt-2 w-fit">
              Save changes
            </Button>
          </div>
        </div>
        <div className="space-y-6">
          <div className="rounded-xl border bg-card p-6">
            ...
            <div className="mt-4 space-y-4">
              <Button variant="outline" onClick={changePw}>
                Change password
              </Button>
              <div className="flex items-center justify-between">
                <Label>Two-factor authentication</Label>
                <Switch
                  checked={f.two_factor_enabled}
                  onCheckedChange={(v) => {
                    setF({ ...f, two_factor_enabled: v });
                  }}
                />
              </div>
            </div>
  ...
  const submit = useMutation({
    mutationFn: async () => {
      ...
      notifyByEmail({
        ...
        intro:
          "Our compliance team is reviewing your identity verification. We'll email you when it's complete.",
        rows: [
  ...
  return (
    <div className="rounded-xl border bg-card p-6">
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="rounded-full border bg-muted px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide capitalize">
          {status}
        </span>
      </div>
      <p className="mt-2 text-sm text-muted-foreground">
        Your daily transaction limit is{" "}
        <span className="font-medium text-foreground">
          {formatUSD(profile?.transaction_limit ?? 5000)}
        </span>
        . Verifying your identity can raise it.
      </p>
      ...
      {locked ? (
        ...
        <div className="mt-4 space-y-3">
          ...
          <div>
            <Label>Selfie holding your document</Label>
            <Input
              type="file"
              accept="image/*"
