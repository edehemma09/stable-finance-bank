...
function Alerts() {
  ...
  const [state, setState] = useState({
    large: prefs?.large_txn_threshold ?? 500,
    low: prefs?.low_balance_threshold ?? 100,
    login: prefs?.login_alerts ?? true,
  });
  useEffect(() => {
    if (prefs)
      setState({
        large: prefs.large_txn_threshold,
        low: prefs.low_balance_threshold,
        login: prefs.login_alerts,
      });
  }, [prefs]);
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase
        .from("alerts_prefs")
        .update({
          large_txn_threshold: Number(state.large),
          low_balance_threshold: Number(state.low),
          login_alerts: state.login,
        })
        .eq("user_id", u.id);
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["me", "alert_prefs"] });
    },
  });
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
          ...
          <div className="mt-4 space-y-4">
            <div>
              ...
              <Input
                type="number"
                value={state.large}
                onChange={(e) => setState((s) => ({ ...s, large: Number(e.target.value) }))}
              />
            </div>
            <div>
              ...
              <Input
                type="number"
                value={state.low}
                onChange={(e) => setState((s) => ({ ...s, low: Number(e.target.value) }))}
              />
            </div>
            <div className="flex items-center justify-between">
              <Label>New device login alerts</Label>
              <Switch
                checked={state.login}
                onCheckedChange={(v) => setState((s) => ({ ...s, login: v }))}
              />
            </div>
        ...
        <div className="rounded-xl border bg-card">
          <div className="border-b p-4">
            <h2 className="font-display text-lg">Recent alerts</h2>
          </div>
