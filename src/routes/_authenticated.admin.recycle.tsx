...
function RecycleBin() {
  ...
  const purge = useMutation({
    ...
    onSuccess: () => {
      toast.success("Archive entry purged");
  ...
  return (
    <div className="p-4 md:p-8">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Deleted customers are archived here with a snapshot of their record at deletion time.
        Purging an entry removes the archive permanently.
      </p>
          ...
          return (
            <div key={d.id} className="rounded-xl border bg-card p-5">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <p className="font-mono text-xs text-muted-foreground">
                    {d.email} · @{d.username ?? "—"} · deleted{" "}
                    {new Date(d.deleted_at).toLocaleString()}
                  </p>
                </div>
                <AlertDialog>
                  <AlertDialogTrigger asChild>
                    <Button size="sm" variant="destructive" disabled={purge.isPending}>
                      Purge permanently
                    </Button>
                  </AlertDialogTrigger>
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        This permanently removes the archived snapshot for {d.email}. It cannot be
                        undone.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                      <AlertDialogCancel>Cancel</AlertDialogCancel>
                      <AlertDialogAction onClick={() => purge.mutate(d.id)}>
                        Purge
                      </AlertDialogAction>
                    </AlertDialogFooter>
                  </AlertDialogContent>
                ...
                {(snap.accounts ?? []).map((a, i) => (
                  <div key={i} className="flex justify-between rounded-lg border px-3 py-2">
                    <span className="capitalize">
                      {a.nickname ?? a.type} ••{String(a.account_number).slice(-4)}
                    </span>
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                {snap.transactions_count ?? 0} transactions archived
              </p>
            </div>
          );
        })}
