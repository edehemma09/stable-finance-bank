    ...
    <div className="container-page py-6 md:py-10">
      ...
      <div className="mt-6 space-y-8">
        {Object.entries(groups).map(([type, list]) => (
          <section key={type}>
            <h2 className="mb-3 text-xs font-semibold uppercase tracking-widest text-muted-foreground">{type}</h2>
            <div className="grid gap-3 md:grid-cols-2">
              {list.map((a) => (
                <Link key={a.id} to="/app/accounts/$id" params={{ id: a.id }} className="rounded-2xl border bg-card p-5 transition hover:border-accent">
                  ...
                  <p className="mt-2 font-mono text-xs text-muted-foreground">•••• {a.account_number.slice(-4)}</p>
                </Link>
              ))}
            </div>
          </section>
        ))}
        {accounts.length === 0 && <p className="rounded-xl border border-dashed p-8 text-center text-sm text-muted-foreground">No accounts yet.</p>}
      </div>
    </div>
  );
}
