  ...
  const create = useMutation({
    mutationFn: async (v: { subject: string; category: string; body: string }) => {
      ...
      const { data: t, error } = await supabase
        ...
        .insert({ user_id: u.id, subject: v.subject, category: v.category })
        .select()
        .single();
      if (error) throw error;
      await supabase
        .from("ticket_messages")
        .insert({ ticket_id: t.id, author_id: u.id, body: v.body, is_staff: false });
    },
    onSuccess: () => {
      ...
      <div className="mt-6 space-y-3">
        ...
        {tickets.map((t) => (
          <Link
            ...
            to="/app/support/$id"
            params={{ id: t.id }}
            className="flex items-center justify-between rounded-xl border bg-card p-4 hover:border-accent"
          >
            <div>
...
function NewTicketForm({
  ...
  onSubmit: (v: { subject: string; category: string; body: string }) => void;
  pending: boolean;
}) {
  ...
  return (
    <form
      onSubmit={(e) => {
        e.preventDefault();
        onSubmit({ subject: s, category: c, body: b });
      }}
      className="space-y-3"
    >
      <div>
      ...
      <div>
        ...
        <Select value={c} onValueChange={setC}>
          ...
          <SelectContent>
            {["general", "account", "transfer", "card", "dispute", "technical"].map((k) => (
      ...
      <div>
        <Label>Message</Label>
        <Textarea rows={5} value={b} onChange={(e) => setB(e.target.value)} required />
      </div>
      <DialogFooter>
